...
def send_photo(id=None, text=None, photo=None, keyboard=None, render=None):
    if photo:
    ...
    for event in longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW:
            if event.from_user:
                ...
                if "начать" in msg:
                    vk.method("messages.send", {"peer_id": id, "message": '', "attachment": "photo-209576287_457239928",
                              'keyboard': kb.keyboard_main.get_keyboard(), "random_id": 0})
            ...
            if event_type in kb.slov:
                ...
                if event in CALLBACK_TYPES:
                    ...
                    with open('raspis.json', 'r', encoding='utf-8') as f:
                        data = json.load(f)
                    messages_edit(event, f_toggle, f"Расписание для {text}", kb.render(
                        course), data['list'][text], unknown=True)
